...
plaus_pts <- import(here("outputs","plaus_pts.Rdata"))
oderes <- as.data.frame(ode(unlist(plaus_pts[1,])))
...
y1 = ggplot(filter(states, time == 2008 & var %in% c("MIN", "SUB", "CLN", "RxCLN", "TRE"))) +
  geom_col(aes(x = var, y = pval)) +
  labs(title = '2008', x = 'State', y = 'Percentage') +
...
y2 = ggplot(filter(states, time == 2018 & var %in% c("MIN", "SUB", "CLN", "RxCLN", "TRE"))) +
  geom_col(aes(x = var, y = pval)) +
  labs(title = '2018', x = 'State', y = 'Percentage') +
  geom_vline(xintercept = 3.5, color = "black", linetype = "dashed", size = 1) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
