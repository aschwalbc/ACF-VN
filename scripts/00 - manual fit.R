...
colnames(parameters) <- c('val','parameter')
rownames(parameters) <- NULL
parameters <- data.frame(parameter = parameters$parameter, low  = parameters$val, med = parameters$val, hig = parameters$val)
export(parameters, here("data","parameters_manual.Rdata"))
rm(parameters)
...
# 1.3 Parameter table output
rangetable <- data.frame(matrix(unlist(ranges), ncol = 2, byrow = TRUE))
colnames(rangetable) <- c("low","high")
parmtable <- data.frame(parameter = names(parms), value = unname(parms), rangetable)
table <- gridExtra::tableGrob(parmtable)
